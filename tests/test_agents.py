...
def test_analysis_has_page_citations(tmp_path: Path):
    ...
    result = ESGAnalysisPipeline(store).run(
...
def test_document_filter(tmp_path: Path):
    ...
    result = ESGAnalysisPipeline(store).run("carbon emissions", 5, ["b"], mode="qa")
...
def test_greenwashing_target_without_baseline(tmp_path: Path):
    ...
    result = ESGAnalysisPipeline(store).run(
        "Review climate target and greenwashing", 5, mode="audit"
    )
